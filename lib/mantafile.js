...
MantaFile.prototype.info = function info(opts, cb) {
  ...
  this.client.info(this.path, function(err, info) {
    if (err)
      return cb(err);

    info.md5 = new Buffer(info.md5, 'base64').toString('hex');
...
MantaFile.prototype.ftw = function ftw(opts, cb) {
  ...
  this.client.ftw(this.path, opts, function(err, res) {
    if (err)
      return cb(err);
    ...
    process.nextTick(function() {
      res.on('entry', function(d) {
        if (d.type !== 'object')
          return;
        ee.emit('file', new MantaFile(path.posix.join(self.path, d.name), self.client));
      });
      res.on('end', function() {
        ee.emit('end');
      });
